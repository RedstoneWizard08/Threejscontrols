...
import { getVelocities } from "./util";
...
export class FirstPersonController extends BasicController {
    ...
    public constructor(player: any, camera: any, element: HTMLElement) {
        super(player, camera, element);
    ...
    public attach(): void {
        ...
        document.addEventListener("pointerlockchange", this.updatePointerLock.bind(this));
    }
    ...
    public detatch(): void {
        ...
        document.removeEventListener("pointerlockchange", this.updatePointerLock.bind(this));
    }
    ...
    private onKeyDown(event: KeyboardEvent): void {
        if (this.locked) switch (event.code) {
            case "KeyW":
            case "ArrowUp":
    ...
    protected isGrounded(): boolean {
        ...
        const rayCaster = new Raycaster(this.player.position, DOWN_DIRECTION, 0, 10);
        rayCaster.set(this.player.position, DOWN_DIRECTION);
